...
function twentyfourteen_body_classes( $classes ) {
	...
	if ( ( ! is_active_sidebar( 'sidebar-2' ) )
		...
		|| is_page_template( 'page-templates/contributors.php' )
         || is_page_template( 'template_grid_view.php' )
		|| is_attachment() ) {
		$classes[] = 'full-width';
	}
...
function create_post_type() {
  register_post_type( 'revision_slider',
    array(
      'labels' => array(
        'name' => __( 'Main Category Pages' ),
        'singular_name' => __( 'revision_slider' )
      ),
      'public' => true,
      'has_archive' => true,
    ...
    register_taxonomy_for_object_type( 'category', 'revision_slider' );
}

function wpa_events_on_category_pages( $query ) {
    if ( $query->is_category() && $query->is_main_query() ) {
        $query->set( 'post_type', array( 'post', 'revision_slider' ) );
    }
}
...
$max_posts = get_theme_mod( 'num_posts_' . $layout, 5 );
...
'order' => 'DESC',
'post_status' => 'publish',
    'post_type' => array( 'post', 'revision_slider'),
);
...
function wpse_filter_post_class( $classes ) {
    ...
    $my_post_class = 'pathfinder-column';
